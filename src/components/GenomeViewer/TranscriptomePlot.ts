...
import { Transcriptome, Dimensions, D3Grid, GridConfig } from 'sparrowgenomelib';
...
export class TranscriptomePlot {
    ...
    constructor(svgElement: d3.Selection<SVGSVGElement, unknown, null, undefined>,
        ...
        this.positionRange = data.positionRange; // Store the position range
    ...
    public plot(): any[] {
        let tx_idx = 0;
        // Calculate the genome range to use (either from positionRange or full genome)
        const minPosition = this.positionRange ? this.positionRange[0] : 0;
        const maxPosition = this.positionRange ? this.positionRange[1] : this.transcriptome.getEnd();
        const genomeLength = maxPosition - minPosition;
            ...
            for (const transcript of transcripts) {
                ...
                if (txPlotSvg) {
                    ...
                    const txPlot = new TranscriptPlot(txPlotSvg, {
                        ...
                        genome_length: genomeLength, // Use calculated genome length
                        transcript: transcript,
                        positionRange: this.positionRange // Pass the position range
                    });
                    txPlot.plot();
                }
                tx_idx += 1;
